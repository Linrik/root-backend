...
<Defaultboxcol>
    {commentList !== null ?
        ...
        return(
            <Comment key={comment.user+index}
                comment={comment} index={index}></Comment>
        )
    }) :
        ""}
        {isLoggedIn &&
    ...
    <Defaultbox sx={{justifyContent:'space-between'}}>
        <TextField required value={text} onChange={onTextChange}
            label={t('write_comment')} sx={{flexGrow:6}}> </TextField>
